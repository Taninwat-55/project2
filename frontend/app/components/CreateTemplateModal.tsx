...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: TemplateData) => void; // Callback för att uppdatera föräldrakomponentens UI
}
...
export default function CreateTemplateModal({
  ...
  // Temporär state för den ingrediens man håller på att skriva in just nu
  ...
  // --- BERÄKNINGAR ---
  // Räknar ut totalen för hela mallen baserat på alla tillagda ingredienser
  ...
  // Data för förhandsgranskningen av makrofördelningen (Protein, Kolhydrater, Fett)
  const chartData = {
    datasets: [
      {
  ...
  // Lägger till den aktuella ingrediensen i listan och nollställer input-fälten
  const addIngredient = () => {
  ...
  // Sparar hela mallen till databasen via en Server Action
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-2 md:p-4 text-left">
      <div className="relative w-full max-w-5xl bg-zinc-900 border border-zinc-800 rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 shadow-2xl overflow-y-auto max-h-[95vh] custom-scrollbar">
        <button
          onClick={onClose}
          className="absolute top-6 right-6 md:top-8 md:right-10 text-zinc-500 hover:text-white transition z-10"
        >
          <X size={24} />
        </button>

        <div className="text-center mb-6 md:mb-10 text-left">
          ...
          <p className="text-zinc-500 text-[10px] md:text-sm mt-2 font-medium italic uppercase tracking-widest">
            Combine ingredients into a saved favorite.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10">
          {/* INPUT-SEKTION: Här skriver användaren in namn och ingredienser */}
          <div className="space-y-4 md:space-y-6">
            <div className="bg-black/20 p-4 md:p-6 rounded-3xl border border-zinc-800/50">
              <label className="text-[9px] font-black text-zinc-500 uppercase mb-2 block tracking-widest text-left">
                Template Name
              </label>
              <input
                type="text"
            ...
            <div className="bg-black/20 p-4 md:p-6 rounded-3xl border border-zinc-800/50 space-y-4">
              <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 mb-2 text-left">
                Add Ingredient
              </h3>
              <input
                ...
                onChange={(e) =>
                  setCurrentIng({ ...currentIng, name: e.target.value })
                }
              ...
              />
              {/* Rutnät för makronutrienter */}
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div>
                  <label className="text-[9px] font-black text-zinc-500 uppercase mb-1 block tracking-widest px-1 text-left">
                    Calories
                  </label>
                  <input
                    type="number"
                    placeholder="kcal"
                    value={currentIng.kcal}
                    onChange={(e) =>
                      setCurrentIng({ ...currentIng, kcal: e.target.value })
                    }
                    className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={currentIng.p}
                    onChange={(e) =>
                      setCurrentIng({ ...currentIng, p: e.target.value })
                    }
                    className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white outline-none"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={currentIng.c}
                    onChange={(e) =>
                      setCurrentIng({ ...currentIng, c: e.target.value })
                    }
                    className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={currentIng.f}
                    onChange={(e) =>
                      setCurrentIng({ ...currentIng, f: e.target.value })
                    }
                    className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white outline-none"
                  />
                </div>
          ...
          {/* PREVIEW-SEKTION: Visar sammanställd data och listan med tillagda varor */}
          <div className="flex flex-col min-h-0 text-left">
            <div className="bg-black/20 p-6 md:p-8 rounded-3xl border border-zinc-800/50 flex-grow flex flex-col items-center justify-between relative overflow-hidden">
              <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 mb-4 text-left w-full">
                Template Preview
              </h3>
              ...
              {/* LISTA MED TILLAGDA INGREDIENSER */}
              <div className="w-full space-y-2 max-h-[180px] md:max-h-[240px] overflow-y-auto pr-2 custom-scrollbar text-left">
                {ingredients.map((ing, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between items-center p-3 bg-zinc-800/30 border border-zinc-700/50 rounded-xl group"
                  >
                    <div className="text-left">
                      <div className="text-xs font-bold text-white uppercase tracking-tight">
                        {ing.name}
                      </div>
                      <div className="text-[9px] font-black text-zinc-500 uppercase italic mt-0.5">
                        {ing.kcal} kcal
                      </div>
                    </div>
                    <button
                ...
                {ingredients.length === 0 && (
                  <div className="py-10 text-[10px] uppercase font-black text-zinc-700 tracking-tighter italic text-center">
                    No fuel added yet...
                  </div>
                )}
              </div>
        ...
        {/* FOOTER: Bekräfta eller avbryt */}
        <div className="flex flex-col md:flex-row justify-center gap-3 mt-8 md:mt-12">
          <button
            type="button"
            onClick={onClose}
            className="order-2 md:order-1 px-8 py-3 bg-zinc-800 hover:bg-zinc-700 rounded-full font-black text-[9px] uppercase tracking-[0.2em] transition text-zinc-400"
          >
            Cancel
          </button>
          <button
