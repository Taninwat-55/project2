    ...
    <>
      ...
      <div className="min-h-screen bg-black text-white py-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          ...
          {/* Global Hero Status */}
          <div className={`transition-all duration-700 p-1 rounded-[3rem] mb-16 border ${isScanning ? "border-white/5 bg-white/5" : "border-green-500/20 bg-green-500/5"
            }`}>
            <div className="bg-[#080808] rounded-[2.8rem] p-8 flex flex-col md:flex-row items-center gap-8">
              <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center transition-all duration-700 ${isScanning ? "bg-blue-500/10 text-blue-500" : "bg-green-500/20 text-green-500 shadow-[0_0_30px_-5px_rgba(34,197,94,0.3)]"
                }`}>
            ...
            {services.map((service) => (
              <div key={service.id} className="group relative">
                <div className="flex items-center justify-between mb-4 px-2">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${isScanning ? "bg-white/5 text-gray-600" : "bg-green-500/10 text-green-500"
                    }`}>
                ...
                <div className="flex gap-[4px] h-10 items-center justify-between">
                  {uptimeData.map((day) => (
                    <div
                      key={day.id}
                      className={`flex-1 rounded-full transition-all duration-1000 ease-out ${isScanning
